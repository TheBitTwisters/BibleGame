...
def check(request):
    ...
    if teams:
        ...
        for team in teams:
            ...
            index += 1
    if question:
        response_question = { 'id': question.id, 'question_text': question.question_text, 'type': question.type, 'fails': question.fails, 'choices': {} }
    if choices:
        index = 0
        current_points = 0
        for choice in choices:
            ...
            if choice.mark:
                current_points += choice.score
            index += 1
        response_question['score'] = current_points
    return JsonResponse({
        'game': response_game,
